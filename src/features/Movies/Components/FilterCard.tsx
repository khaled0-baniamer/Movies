...
import { useGetGenreMovies, useGetLanguages } from "@/hooks";
import { useEffect, useState } from "react";
import { useAppDispatch } from "@/store";
import { setGenres, setLanguage } from "../slice/movieFilterSlice";
import {
  useMovieMinUserVoteFilter,
  useMovieReleaseMovieDateFilter,
  useMovieRuntimeFilter,
  useMovieVoteAverageFilter,
} from "../hooks";
...
const FilterCard: React.FC<Props> = ({}) => {
  ...
  const { data: genre } = useGetGenreMovies();
  const { endDate, handleChangeDate, startDate } =
    useMovieReleaseMovieDateFilter();
  const { handleChangeAverageVote, max, min } = useMovieVoteAverageFilter();
  const { handleChangeMinUserVote, minUserVote } = useMovieMinUserVoteFilter();
  const { handleChangeRuntime, maxRuntime, minRuntime } =
    useMovieRuntimeFilter();
